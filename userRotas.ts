...
rotas.post("/user/addUser", (req: Request, res: Response) => {
  try {
    ...
    if ((novoUsuario.nome as string).length < 4) {
      throw new Error(
        "O tamanho do Nome deve ser maior ou igual a 4 caracteres!"
      );
    }
    ...
    if (!userExist) {
      ...
      return res
        .status(201)
        .send({ msg: `Usuário ${novoUsuario.nome} adicionado com sucesso!` });
    } else {
      return res.status(409).send({ msg: "Este e-mail já existe!" });
    }
  } catch (error: unknown) {
    // aqui esta capturando um erro genérico

    if (error instanceof Error) {
      // esse if esta  verificando se o erro é uma instancia da classe error
